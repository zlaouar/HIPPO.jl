...
"""
@with_kw struct HippoGridWorld <: MDP{GWPos, Symbol}
...
function POMDPs.states(mdp::HippoGridWorld)
    ss = vec(GWPos[GWPos(x, y) for x in 1:mdp.size[1], y in 1:mdp.size[2]])
    push!(ss, GWPos(-1,-1))
    return ss
end

function POMDPs.stateindex(mdp::HippoGridWorld, s::AbstractVector{Int})
    if all(s.>0)
...
POMDPs.initialstate(mdp::HippoGridWorld) = GWUniform(mdp.size)

# Actions

POMDPs.actions(mdp::HippoGridWorld) = (:up, :down, :left, :right)
...
POMDPs.actionindex(mdp::HippoGridWorld, a::Symbol) = aind[a]


# Transitions

POMDPs.isterminal(m::HippoGridWorld, s::AbstractVector{Int}) = any(s.<0)

function POMDPs.transition(mdp::HippoGridWorld, s::AbstractVector{Int}, a::Symbol)
...
function inbounds(m::HippoGridWorld, s::AbstractVector{Int})
    return 1 <= s[1] <= m.size[1] && 1 <= s[2] <= m.size[2]
end

# Rewards

POMDPs.reward(mdp::HippoGridWorld, s::AbstractVector{Int}) = get(mdp.rewards, s, 0.0)
POMDPs.reward(mdp::HippoGridWorld, s::AbstractVector{Int}, a::Symbol) = reward(mdp, s)


# discount

POMDPs.discount(mdp::HippoGridWorld) = mdp.discount

# Conversion
function POMDPs.convert_a(::Type{V}, a::Symbol, m::HippoGridWorld) where {V<:AbstractArray}
    convert(V, [aind[a]])
end
function POMDPs.convert_a(::Type{Symbol}, vec::V, m::HippoGridWorld) where {V<:AbstractArray}
    actions(m)[convert(Int, first(vec))]
end

# deprecated in POMDPs v0.9
POMDPs.initialstate_distribution(mdp::HippoGridWorld) = GWUniform(mdp.size)