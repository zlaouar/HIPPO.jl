...
hipposim = HIPPOSimulator(msim=pomdp, planner=planner, up=particle_up, b=particle_b, sinit=sinit, 
                            dt=1/10, max_iter=max_iter, display=false, verbose=false)
bsim = UnifiedBaselineSimulator(msim=pomdp, sinit=sinit, dt=1/4, max_iter=max_iter, display=false, verbose=false)
